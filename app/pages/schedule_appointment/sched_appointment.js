  ...
  fetch(`${config.API_URL}/appointments`, {
    ...
    .then((data) => {
      if (data.status === "success") {
        ...
        divResponse.innerHTML = `
                    ...
                    <p>Para ver todos los detalles ve a la pagina de premios </p>
                    <a href="../awards/awards.html" class="btn btn-primary">Ir a Premios</a>
                    `;
      } else {
        const divResponse = document.getElementById("step-3");
        divResponse.innerHTML = `
    ...
    .catch((error) => {
      ...
      divResponse.innerHTML = `
                ...
                <h3 class="text-center">Hubo un error al agendar la cita</h3>
                <p>Ocurrió un error, intentalo más tarde</p>
                <a href="../awards/index.html" class="btn btn-primary">Ir a Premios</a>
                `;
    });
}
